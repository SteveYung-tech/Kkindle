...
public static class ReadingMaterialsExport
{
    ...
    {
        var builder = new StringBuilder("# Kkindle 阅读资料\n\n");
        AppendGroups(records, (source, book, items) =>
        {
            ...
            foreach (var item in items)
            {
                ...
                if (!string.IsNullOrWhiteSpace(item.Note)) builder.AppendLine().Append("笔记：").AppendLine(item.Note);
                if (!string.IsNullOrWhiteSpace(item.Location)) builder.AppendLine().Append("位置：").AppendLine(item.Location);
                if (item.UpdatedAt is { } time) builder.Append("时间：").AppendLine(time.ToLocalTime().ToString("yyyy-MM-dd HH:mm"));
                builder.AppendLine();
            }
        });
        if (records.Count == 0) builder.AppendLine("暂无划线与笔记。");
    ...
    public static string BuildPlainText(IReadOnlyList<ReadingMaterialRecord> records)
    {
        ...
        AppendGroups(records, (source, book, items) =>
        {
            ...
            foreach (var item in items)
            {
                ...
                if (!string.IsNullOrWhiteSpace(item.Note)) builder.Append("笔记：").AppendLine(item.Note);
                if (item.UpdatedAt is { } time) builder.Append("时间：").AppendLine(time.ToLocalTime().ToString("yyyy-MM-dd HH:mm"));
                builder.AppendLine();
            }
        });
        ...
        Action<string, string, IReadOnlyList<ReadingMaterialRecord>> append)
    {
        ...
        foreach (var bookGroup in sourceGroup.GroupBy(item => item.BookTitle, StringComparer.CurrentCultureIgnoreCase))
            append(sourceGroup.Key == ReadingMaterialSource.Local ? "本地书籍" : "Kindle", bookGroup.Key, bookGroup.ToArray());
    }
}
