...
public sealed partial class EpubBookContentService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var chapterIndex = 0; chapterIndex < document.Chapters.Count; chapterIndex++)
        {
            ...
            chunks.AddRange(CreateChunks(text, chapterIndex, chapterTitle, relativePath));
        }

        await _readerData.ReplaceBookChunksAsync(book.Id, file.Id, file.Sha256, chunks, cancellationToken);
        return chunks.Count;
    }
    ...
    private static void AppendElementText(XElement element, StringBuilder builder)
    {
        ...
        if (isBlock && builder.Length > 0) builder.Append('\n');
