...
完整测试构建（Windows，224 项 = 190 可移植 + 28 Windows + 6 平台公共）：
...
dotnet publish src\Kkindle.Desktop.Windows\Kkindle.Desktop.Windows.csproj `
  ...
  -o artifacts\Kkindle-debug-win-x64-single --no-restore
Copy-Item artifacts\Kkindle-debug-win-x64-single\Kkindle.exe `
  artifacts\Kkindle-debug-win-x64-latest\Kkindle.exe -Force
```

同步 `latest` 时只覆盖 EXE，保留其旁边的 `data\`、`backups\` 和崩溃日志。
...
- 2026-08-17 的 `master` 已包含阶段 7 跨平台提交；本次提交继续合入 Kreader Windows 边框、书签、目录/子章节、分页布局、键鼠导航、进度恢复、快速按键稳定性与相关测试。
...
| `Kkindle.Tests` | 3,000+ 行，190 项 | 可移植测试 |
...
- [x] 阶段 4 核心切片回归后 Debug x64 测试 192 项全通过（2026-08-17 增至 224 项）
...
- [x] 测试基线已在 2026-08-17 增至：`Kkindle.Tests` 190 + `Kkindle.Platform.Common.Tests` 6 + `Kkindle.Tests.Windows` 28 = 224 项 Debug。
...
- 回归基线：**224 项测试**必须始终全绿（190 可移植 + 28 Windows + 6 平台公共）
...
- [x] 2026-08-17 已重新构建并运行 224 项测试（190 + 28 + 6）；Windows Avalonia 单文件 Debug EXE 启动保持运行 6 秒且未新增崩溃日志。
