...
- Kreader 全书搜索结果滚动条支持自动隐藏并统一使用浅灰色滑块，搜索结果卡片右侧留出滑块避让；左侧底部“返回书架”按钮铺满整块底栏、与右侧底栏同高，外框采用与右侧底栏一致的 `#E2E2DE` 浅灰线。
...
- 当前 `master` 工作区干净；本地比 `origin/master` 超前 5 个提交（均未推送）；构建输出由 `.gitignore` 排除。
