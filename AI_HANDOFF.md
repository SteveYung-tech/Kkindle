...
> 当前迁移状态（2026-08-15）：Avalonia 阶段 2（本地书库）和阶段 3（Kindle 设备、阅读资料、Z-Library、设置与备份）已完成，阶段 4（Kreader 阅读器）已完成核心交互切片、像素级布局重构与 22 项功能对等缺口全量移植（见 10.5），阶段 5 发布入口已切到 Avalonia Windows 启动头，阶段 6 已完成 Windows 侧可移植性验证。2026-08-15 在**未提交的工作区**中新增三端落地：`Kkindle.Platform.Common`（挂载 U 盘 Kindle 服务 + AES-GCM 密钥保护基类）、`Kkindle.Platform.Linux/MacOS`（Secret Service/Keychain、XDG/Application Support 数据目录、udisksctl/diskutil 弹出）、`Kkindle.Desktop.Linux/MacOS` 启动头、`Kkindle.Platform.Common.Tests`、三端发布脚本（`build-linux-release.sh`/`build-macos-release.sh`）、`cross-platform.yml` 校验 workflow、`release.yml` 三端构建 + 聚合 GitHub Release、`docs/cross-platform.md`；Calibre 全量去捆绑（`CalibreExecutableLocator` 三端发现 + `CalibreSetupService` 用户手动安装 Calibre/KFX Input）；阅读器收尾（EPUB 远程脚注图消毒 v8、极简目录 marker 渲染修复、搜索结果条目样式、筛选面板移除动画）；**窗口关闭默认先退书架**（阅读中点击标题栏 X / Alt+F4 只关闭 Kreader 回到主界面，Avalonia 与 WinUI 参照版同步，见 10.5 阶段 7）。WinUI 版本仍保留作迁移参照。尚未完成：同分辨率/DPI 像素级截图验收、真实 EPUB/PDF/AI/脚注人工验收、Linux/macOS 真实桌面与真实 Kindle 验收；Windows Debug EXE 仍仅作为迁移调试版。
...
- 分支 `master` 与 `origin/master` 同步于 `f3b536e`（2026-08-15 18:57「feat: animate book cover hover」，共 168 个提交）；`v0.5.2`（`5daf140`）已推送并自动发版。**工作区包含当日新增但尚未提交的改动**：三端平台/启动头与发布体系、Calibre 去捆绑、阅读器收尾、窗口关闭默认退书架（详见第 9 节清单与 10.5 阶段 7）。
...
- 测试：Debug 2026-08-15 共 **215 项**全部通过（0 失败、0 跳过），分布在三个项目：`Kkindle.Tests` 181 项（`net8.0`，可跨平台）+ `Kkindle.Tests.Windows` 28 项（`net8.0-windows`，WPD/MTP 设备测试）+ `Kkindle.Platform.Common.Tests` 6 项（`net8.0`，新增的 AES-GCM 与挂载 U 盘设备服务测试）。
- 当前 Avalonia 调试 EXE：Windows 为 `src\Kkindle.Desktop.Windows\bin\x64\Debug\net8.0-windows10.0.19041.0\win-x64\Kkindle.exe`；Linux/macOS 启动头需在对应平台构建（见第 6 节）。Windows EXE 已完成构建与启动保持运行检查，但尚未作为 UI/功能 1:1 对等验收包。
...
- 已启动 WinUI 3 → Avalonia 迁移（Windows 优先，架构上预留 Linux/Mac），计划与进度见第 10 节；阶段 4 功能对等缺口已全量移植（见 10.5），阶段 7 已完成三端落地（未提交），WinUI 版继续保留。
...
1. **WebView 安全基线**：EPUB 在落盘阶段剥离 `<script>`、`on*` 属性、`javascript:`/`data:` URL 与外部资源，注入 CSP `<meta>`（`default-src 'none'` + 自研 nonce）只放行自己的注入脚本；导航白名单限当前 EPUB 缓存/PDF；`EnableDevTools=false`；右键菜单由页面级 `contextmenu` preventDefault 取消（Avalonia adapter 无托管 WebView2 设置对象，`AreDefaultContextMenusEnabled=false` 由页面事件替代）。
...
8. **阅读器脚本模块**：导航/分页/外观/水波分别在 `ReaderNavigationScripts.cs`、`ReaderPaginationScripts.cs`、`ReaderAppearanceScripts.cs`、`ReaderWaveScripts.cs`；动画参数（水波 560ms、32 条带等）只改常量。旧纸质翻页实现已删除，不要恢复。Avalonia 与 WinUI 共享同一份脚本文件（`Kkindle.App.csproj` 以链接方式引入）。
...
完整测试构建（Windows，215 项 = 181 可移植 + 28 Windows + 6 平台公共）：
...
- 当前 `master` 与 `origin/master` 同步于 `f3b536e`（168 个提交，工作区相对远端无超前/落后）；构建输出由 `.gitignore` 排除。
- **工作区存在未提交改动**（2026-08-15，尚未创建提交）：
  - 新增：`Kkindle.Platform.Common`、`Kkindle.Platform.Linux`、`Kkindle.Platform.MacOS`、`Kkindle.Desktop.Linux`、`Kkindle.Desktop.MacOS`、`tests/Kkindle.Platform.Common.Tests`、`Kkindle.Infrastructure/CalibreExecutableLocator.cs`、`CalibreSetupService.cs`、`scripts/build-linux-release.sh`、`scripts/build-macos-release.sh`、`.github/workflows/cross-platform.yml`、`docs/cross-platform.md`；`Kkindle.sln` 已登记新项目。
  - 修改：`release.yml`（三端 job + 聚合 Release）、`Build-Release.ps1`（移除 `-CalibreRuntime`）、WinUI 与 Windows 头 csproj（移除 Calibre 捆绑）、`AppRootConfiguration.cs`、`BookFormatConversionService.cs`、`EpubReaderPreparationService.cs`（脚注图消毒 v8）、`Kkindle.Infrastructure.csproj`（InternalsVisibleTo Platform.Common/Tests）、Avalonia `App.axaml(.cs)`/`MainWindow.*`/`NativeWebViewReaderHost.cs`（设置页 Calibre 安装按钮、ComboBoxItem/搜索结果黑白样式、筛选面板去动画、WPE/GTK 探测、窗口关闭默认退书架）、`MainWindow.xaml.cs`（WinUI 同步窗口关闭默认退书架）、README 与相关测试。
...
> 状态：**阶段 0、1、2、3 已完成，阶段 4 已完成（核心交互切片 + 像素级布局重构 + 功能对等缺口全量移植），阶段 5 发布入口已完成，阶段 6 Windows 侧验证已完成，阶段 7 三端落地已在工作区完成（未提交，见 10.5 阶段 7）**。目标是把 UI 层从 WinUI 3 换成 Avalonia，先在 Windows 上达到功能对等，同时把平台相关代码隔离干净，使后续接 Linux/Mac 只需新增平台实现、不改业务代码。
>
...
| `Kkindle.Tests` | 3,000 行，181 项 | 可移植测试 |
...
- [x] 阶段 4 核心切片回归后 Debug x64 测试 192 项全通过（后增至 215 项，见阶段 7）
...
**阶段 7：Linux/macOS 三端落地 — 工作区已完成（2026-08-15，未提交）**
...
- [x] 测试：`Kkindle.Tests` 181 + `Kkindle.Platform.Common.Tests` 6 + `Kkindle.Tests.Windows` 28 = 215 项 Debug 全通过；Avalonia Windows Debug 构建 0 警告/0 错误。
...
- 回归基线：**215 项测试**在阶段 0 之后必须始终全绿（181 可移植 + 28 Windows + 6 平台公共）
...
当前结论：功能对等缺口已按 `docs/Kreader功能对比.md` 全量移植，三端平台层与启动头已落地（未提交），但**尚未完成**同分辨率/DPI 像素级截图与真实书籍交互验收，Linux/macOS 尚未真机验收，不得将当前 Debug EXE 标记为 WinUI 旧版的 1:1 对等版本。
...
- [x] 已重新构建、运行 215 项测试（181 + 28 + 6），并启动 Windows Avalonia Debug EXE 保持运行 8 秒后正常退出测试进程。
- [x] 已落地三端平台层/启动头/发布体系与 Calibre 去捆绑（未提交，见 10.5 阶段 7）；**窗口关闭默认先退书架**已在 Avalonia 与 WinUI 参照版同步实现。
...
7. **三端改动未提交**：阶段 7 全部改动仍在工作区，尚未经过三端 CI 与真机验收；跨平台改动量大，建议先行提交并让 `cross-platform.yml` 跑通后再继续后续功能。
