...
> 续作最新基线见第 62～64 节：P1/P2 已全部完成，Kindle 字体/字典支持设备端读写，本地书籍与 Kindle 划线笔记已统一管理，并已加入安装版与 GitHub 自动发版。以下早期章节保留为历史记录，各轮测试数字以最新章节为准。
...
- 当前分支：`master`；完成本轮提交与推送后，本地 `master` 与 `origin/master` 应保持一致。
- GitHub：`git@github.com:kingstacker/Kkindle.git`；推送 `vX.Y.Z` 标签会触发 Windows 自动发版。
- 最新标准发布目录：`src\Kkindle.App\bin\x64\Release\net8.0-windows10.0.19041.0\win-x64\publish\`；2026-08-10 重新发布并验证。
- 最新源码验证：Debug/Release x64 共 136 项测试全部通过；安装版与便携版均已实际生成，安装/卸载冒烟测试通过，SHA-256 校验匹配。
...
- `MainWindow.xaml` 已恢复书架和设备书库主界面；筛选已可用，分类与设置仍是轻量入口，未形成独立页面/数据模型。
...
- [x] 侧栏“笔记与标注”入口：复用当前选中/正在阅读的 EPUB，直接打开 Kreader 的划线与笔记页签。
- [x] 侧栏“导出记录”入口：复用当前 EPUB 的批注数据，直接进入现有 Markdown 导出流程；没有 EPUB 时明确提示。
- [x] 分页模式双页左右显示：更多菜单可切换双页分页；每个视口显示左右两栏，翻页/吸附仍按一个视口步进，并按 `BookFileId` 持久化。
- [x] 书籍格式互转与 Kreader 扩展：右键可用 Calibre 在 EPUB、AZW3、PDF 间转换，结果写回同一本书；PDF 继续使用内置查看器，AZW3 自动转临时 EPUB 后进入 Kreader。
- [ ] 分页模式竖排排版：当前仍强制横排，竖排只在滚动模式生效；需要真实 WebView2 视觉回归后再实现多列横向分页、断点恢复、目录/批注定位和边界翻页。
- [ ] 字体管理与字典管理：侧栏仍是禁用占位入口，当前只有阅读器内置字体选择和本地字体资源，没有独立管理页或词典数据模型。
- [ ] 分类与完整设置页：目前标签筛选可用，分类入口和设置入口仍是轻量提示，尚未建立独立分类/应用设置模型。
- [ ] WebView2 合成岛真实鼠标/滚轮端到端验收：分页分区点击、滚动接章、选区工具栏、竖排目录/fragment 定位仍需交互桌面人工复核。
...
当前工作分支为 `master`；`origin/master` 最新为 `9b1583a`，本地领先 18 个提交，未自动推送。当前工作区无未跟踪文件；构建输出由 `.gitignore` 排除，不纳入版本控制。
...
## 39. Kreader 极简目录模式（2026-08-07）

- 目录面板收起按钮改用与 AI 助手收起按钮完全一致的 `E76C` 图标；完整目录标题栏新增极简目录切换按钮。
- 新增约 30px 宽的极简目录条：每个章节显示为小矩形，悬停显示章节标题，点击直接复用现有 TOC 导航意图跳转；正文只损失很小的左侧宽度。窄条可通过顶部图标恢复完整目录，正文标题栏的目录按钮也可从极简模式恢复完整目录。
- 极简目录复用现有目录筛选数据，打开/关闭阅读器和 PDF 会话清空数据；Zen 模式会暂时隐藏并在退出时恢复之前的目录模式；目录跳转逻辑拆入 `MainWindow.ReaderToc.cs`。
- Release x64 完整解决方案构建 0 警告、0 错误；83 项 Release 测试全部通过；发布版启动存活检查通过并正常关闭；标准便携版已重新发布：`src\Kkindle.App\bin\x64\Release\net8.0-windows10.0.19041.0\win-x64\publish\Kkindle.exe`（2026-08-07 23:23:34）。本轮修改与交接文档一起提交，未 push；未跟踪 `.opencode/` 保留且未加入提交。

## 40. Kreader 极简目录标记与滚轮浏览（2026-08-07）

- 极简目录默认使用白底黑框空心矩形，当前正文对应章节刷新为实心黑矩形；章节切换、目录点击和 WebView 导航完成后会同步当前标记。
- 窄条目录增加显式鼠标滚轮处理，滚动位置按实际可滚动高度钳制，章节数量超过可视矩形数量时可以上下浏览；矩形悬停标题和点击跳转保持不变。
- Release x64 完整解决方案构建 0 警告、0 错误；83 项 Release 测试全部通过；发布版启动存活检查通过并正常关闭；标准便携版已重新发布：`src\Kkindle.App\bin\x64\Release\net8.0-windows10.0.19041.0\win-x64\publish\Kkindle.exe`（2026-08-07 23:33:54）。本轮修改与交接文档一起提交，未 push；未跟踪 `.opencode/` 保留且未加入提交。

## 41. Kreader 极简目录滚轮动画与悬停反馈（2026-08-07）

- 极简目录滚轮改为约 160ms 的缓出平滑滚动；连续滚轮输入以最新目标位置合并，滚动高度按实际可滚动范围钳制，目录隐藏或刷新时会停止动画计时器。
- 鼠标指向极简目录矩形时，矩形通过 `ScaleTransform` 横向平滑放大到约 1.18 倍，移出后恢复原尺寸；当前章节仍使用实心黑矩形，普通章节仍使用黑框空心矩形。
...
## 44. Kreader 极简目录隐藏滑块与矩形裁切修复（2026-08-07）

- 极简目录垂直滚动条恢复隐藏，继续使用已有的鼠标滚轮缓动和实际滚动范围钳制，不再在 30px 窄条内为原生滑块占用布局宽度。
...
## 47. AI 助手、Kindle 邮件与内置字体发布（2026-08-08）

- AI 助手输入区新增思考深度和模型选择控件；兼容 SSE 流式响应，正文与 reasoning 内容均实时渲染。
- DeepSeek 兼容 OpenAI 风格接口，并支持根据当前服务地址自动请求模型列表；模型下拉框会使用服务端返回的模型。
- Kindle 邮件发送功能已接入书籍选择、收件人配置、发送状态和错误反馈流程。
- 全局界面与阅读器默认字体改为京华老宋体；正文仍可在阅读排版设置中切换字体。
- 将版本 3.0 字体文件随应用发布，原生 WinUI 使用应用资源 URI，EPUB 阅读器通过本地 `@font-face` 注入；同时携带字体来源与许可记录。
- 内置字体文件大小为 33,259,644 字节，发布输出中的 SHA-256 为 F7FEF9FC413E9E2343F0BB432C51CCA41C44B8FE37F071DC86B050896AE9F9E2。
- Release x64 完整解决方案构建 0 警告、0 错误；89 项测试全部通过。
- 标准便携版已重新发布：`src\Kkindle.App\bin\x64\Release\net8.0-windows10.0.19041.0\win-x64\publish\Kkindle.exe`；6 秒启动存活检查通过并正常关闭。
- 本轮代码、测试、字体资源与交接文档一并提交，未 push；`.opencode/` 保留为未跟踪目录，不加入提交。

## 48. 脚注悬停浮窗解析与显示修复（2026-08-08）

- 修复脚注目标 ID 位于回链标记 `<a>` 上时只显示 `[1]`、不显示同段落详细内容的问题；解析器现在优先读取最近的段落、列表或脚注内容块。
- 统一脚注目标的文件 URL 与 fragment 规范化，避免预加载链接和悬停实时识别结果因编码或路径表示不同而匹配失败。
- 预加载尚未完成或当前目标未进入预加载列表时，悬停逻辑会对当前脚注目标即时解析，并用版本号与取消令牌防止旧章节结果覆盖新章节。
...
- 修复《策略思维》注 11 等脚注目标文件使用 `&nbsp;` 等 HTML 命名实体时，严格 XML 解析失败并被跳过，导致悬停无浮窗的问题。
- `EpubFootnoteResolver` 保留严格 XHTML 解析，失败后对 HTML 命名实体做安全兼容转换再解析；合法 XML 实体保持原样，脚注路径白名单和 fragment 匹配规则不变。
- 新增包含 `&nbsp;` 的旧式 XHTML 脚注回归测试；Release x64 完整解决方案构建 0 警告、0 错误，91 项测试全部通过。
- 标准便携版已重新发布：`src\Kkindle.App\bin\x64\Release\net8.0-windows10.0.19041.0\win-x64\publish\Kkindle.exe`（2026-08-08 07:53:20），EXE SHA-256 为 `F9349844CEA4FD6004E971B5621EAE35F946737B88A46DD7E0982BC4C1738C3F`。
- 本轮代码、测试、交接文档与发布对应提交，未 push；`.opencode/` 保留为未跟踪目录，不加入提交。

## 50. 侧栏阅读资料入口接通（2026-08-08，工作区未提交）

- 侧栏“笔记与标注”不再是禁用占位；无阅读会话时使用书架当前选中的 EPUB，已有阅读会话时复用当前 EPUB，打开 Kreader 的“划线与笔记”页签。
- 侧栏“导出记录”复用同一 EPUB 会话并直接进入现有 Markdown 批注导出流程；当前书籍没有 EPUB 时不误打开 PDF，而是显示明确提示。
- `ReaderBookSelectionPolicy` 新增 `SelectEpub`，把“阅读资料仅支持 EPUB”的格式判断放到 Core，并新增格式选择回归测试。
- Release x64 完整解决方案构建 0 警告、0 错误；92 项 Release 测试全部通过。
- 本轮未重新发布便携版、未创建提交；工作区无未跟踪文件。分页模式竖排、字体/字典管理、分类/完整设置页以及真实硬件/合成岛交互验收仍按第 6 节保留。

## 51. 正文选区浮窗去边框（2026-08-08，工作区未提交）

- Kreader 正文选区快捷浮窗移除外层黑色边框和各功能按钮的独立边框。
- 复制、划线、批注、AI 解释、搜索之间改用 1px 灰色竖线分隔；悬停和按下底色反馈、原有事件绑定保持不变。
- Release x64 完整解决方案构建 0 警告、0 错误；92 项 Release 测试全部通过。
- 本轮未重新发布便携版、未创建提交。

## 52. 正文分页双页左右显示（2026-08-08，工作区未提交）

- “更多”菜单新增“ 双页显示（仅分页） ”开关；滚动模式下禁用，分页按钮显示“单页/双页”，切换后从当前章节开头重新对齐。
- 分页 CSS 将每个视口分成左右两栏，单页仍保持原有列宽；双页通过一个视口宽度容纳两列和列间距，现有翻页、吸附、断点/目录/批注定位继续按一个视口步进。
- `ReaderLayoutSettings.TwoPageMode` 按 `BookFileId` 持久化；新库建列，旧库启动时通过 `PRAGMA table_info` + `ALTER TABLE` 自动补列，旧设置默认保持单页。
- 排版控件更新和恢复默认不会误清除双页开关；新增列宽、默认值、规范化和持久化回归测试。
- Release/Debug x64 完整解决方案构建 0 警告、0 错误；93 项测试全部通过。
- 本轮未重新发布便携版、未创建提交；`.opencode/` 已按要求删除，工作区无未跟踪文件。

## 53. 正文搜索移到目录侧栏（2026-08-08，工作区未提交）

- 正文顶部工具栏的“搜索”按钮移到“目录”按钮旁，PDF 仍按原逻辑隐藏。
- 整本书搜索结果不再以右上角 Popup 浮层显示，改为占用原目录侧栏；打开时临时展开目录栏，关闭后恢复此前的展开/极简目录状态。
- 搜索索引、Enter 搜索、结果点击跳转、Ctrl+F 和 WebView2 脚本禁用策略均保持不变。
- Release/Debug x64 完整解决方案构建 0 警告、0 错误；93 项测试全部通过。
- 本轮未重新发布便携版、未创建提交；工作区无未跟踪文件。

## 54. 书籍格式互转与 Kreader PDF/AZW3 查看（2026-08-08，工作区未提交）

- 书架卡片和列表的右键菜单新增 EPUB、AZW3、PDF 三种转换入口；转换源会跳过目标格式并按 EPUB/AZW3/PDF 优先级选择，结果通过书库服务写回原书，不创建重复书籍。
- 新增 `BookFormatConversionService`，调用本机 Calibre `ebook-convert.exe`；支持 `KKINDLE_CALIBRE_CONVERT` 覆盖路径、Program Files/Program Files (x86)/PATH 查找、参数列表传递、失败输出截断、取消杀进程和临时文件清理。
- Kreader 的阅读选择策略纳入 AZW3；PDF 继续走 WebView2 内置查看器，AZW3 打开时转换到临时 EPUB，再复用现有目录、分页、搜索和章节导航，原始 AZW3 的书库文件与阅读设置标识不变。
- 新增格式转换策略、AZW3 阅读选择和同书添加新格式测试；Release/Debug x64 完整解决方案构建 0 警告、0 错误，99 项测试全部通过。
- 本机已确认 Calibre `ebook-convert.exe` 可用，版本为 9.12.0；本轮未重新发布便携版、未创建提交，工作区无未跟踪文件。

## 55. 发布包内置 Calibre 转换运行时（2026-08-08，工作区未提交）

- `BookFormatConversionService` 优先查找可执行文件旁的 `Calibre\ebook-convert.exe`，其次兼容 `Calibre2`、系统安装目录、PATH 和 `KKINDLE_CALIBRE_CONVERT` 覆盖路径；调用内置运行时时设置独立的 `CALIBRE_CONFIG_DIRECTORY`，不污染用户的 Calibre 配置。
- `Kkindle.App.csproj` 新增 `KkindleCalibreRuntime` 发布参数和 `CopyBundledCalibreToPublish` 目标；发布时把指定 Calibre 运行时复制到 `publish\Calibre`，用户不需要另行安装 Calibre。
- 发布目录携带 `Calibre\LICENSE` 与 `Calibre-THIRD-PARTY-NOTICE.txt`，标明 Calibre GPLv3 许可及源代码地址。
- 使用本机 Calibre 9.12.0 运行时完成 `ebook-convert --version` 检查和 TXT→EPUB 实际转换检查；运行时共 1341 个文件，约 626.14 MB。

## 56. 左侧导航状态与格式转换进度（2026-08-08，工作区未提交）

- 左侧主功能按钮只有在其子功能被选中后才显示黑色实心矩形；展开/收起主功能不会改变选中主功能，未选中主功能悬停只显示浅灰反馈。
- 左侧导航 `ScrollViewer` 改为隐藏垂直滚动条，保留鼠标滚轮和触控滚动能力。
- `IBookFormatConverter` 新增 `IProgress<FormatConversionProgress>` 回调；Calibre 标准输出中的百分比会被实时解析并更新格式转换浮窗。
- 格式转换浮窗提供“后台”和“取消”；缩小后对应书籍卡片/列表行右下角显示黑白矩形百分比进度块，点击进度块可恢复浮窗；书架刷新或筛选时转换状态按 `BookId` 重新绑定。
- Release 构建和 104 项测试已通过；带内置 Calibre 运行时的便携包已重新发布，`Kkindle.exe` 启动存活检查通过；EXE SHA-256 为 `AE27FFD83284FF81156BC05A88D62B78CB7CEFA4CFAE5EDCCAB3D2A6822CA67E`。

## 57. 格式转换窗口乱码修复（2026-08-08，工作区未提交）

- Calibre 标准输出和错误输出统一按 UTF-8 读取，并过滤终端控制字符；无法解码的进度行回退为稳定的中文状态文字。
- 转换小窗口底部不再直接展示 Calibre 控制台原始文本，转换中统一显示“正在转换…”，百分比继续由进度条和百分比标签实时显示。
- Release 构建 0 警告、0 错误；104 项测试全部通过；带内置 Calibre 运行时的便携包已重新发布，启动存活检查通过。

## 58. 多格式书籍打开选择（2026-08-08，工作区未提交）

- 书籍右键菜单的“打开书籍”改为子菜单，动态列出当前书籍已有的 EPUB、PDF、AZW3；不再弹出独立格式选择窗口。
- 子菜单选项直接打开对应格式；双击和详情“开始阅读”在没有子菜单操作时按 EPUB、PDF、AZW3 优先级直接打开。
- 笔记与导出入口继续直接使用 EPUB；MOBI 等当前不支持内置阅读的格式不进入打开子菜单。
- 新增 ReaderBookSelectionPolicy.GetSupportedFiles 和格式顺序回归测试；Release 构建 0 警告、0 错误，105 项测试全部通过；便携版已重新发布并启动存活检查通过。
- 格式转换小窗口的格式行明确显示 Calibre，转换中的底部状态显示“Calibre 正在转换…”，不再让用户误以为使用了其他转换器。

## 59. 按格式删除书籍文件（2026-08-08，工作区未提交）

- 书架卡片和列表右键新增“删除格式”子菜单，动态显示当前书籍已有的 EPUB、PDF、MOBI、AZW3；删除操作带二次确认。
- 新增 IBookLibraryService.DeleteFileAsync，只删除指定 BookFile 及其磁盘文件，不影响同一本书的其他格式；删除最后一个格式时同时清理书籍记录和封面。
- 正在阅读的格式会拒绝删除并提示先关闭阅读器；删除后书架和详情面板自动刷新。
- 新增数据层回归测试；Release 构建 0 警告、0 错误，106 项测试全部通过；便携版已重新发布并启动存活检查通过。

## 60. 一键删除全部格式（2026-08-09，工作区未提交）

- 书架卡片和列表右键新增独立的“删除全部格式”入口，不需要逐个格式删除。
- 该入口删除整本书的所有格式、书籍记录和封面，并保留二次确认；正在阅读该书时要求先关闭阅读器。
- 详情页“从书库删除”统一复用同一删除逻辑；Release 构建 0 警告、0 错误，112 项测试全部通过；便携版已重新发布并启动存活检查通过。

## 61. 脚注浮窗限制在正文区域（2026-08-09，工作区未提交）

- 脚注浮窗的定位边界从整个窗口改为正文 WebView 的实际可视区域，左右不会越过目录侧栏，顶部和底部也不会越出正文窗口。
- 正文区域变窄或目录展开时，浮窗宽度和最大高度会同步收缩；上下左右定位继续根据脚注锚点自动选择并在正文范围内吸附。
- Release x64 构建 0 警告、0 错误；112 项测试全部通过；带内置 Calibre 运行时的便携版已重新发布，Kkindle.exe 启动存活检查通过；EXE SHA-256 为 `100175D0B7A79708AA43FFDFB78D3E1BB9FF165CEB86F9C951C71445A138E65B`。

## 62. P1/P2 完成与阅读资料统一管理（2026-08-10）

- P1/P2 计划项已全部完成：分类、收藏、阅读状态、批量筛选与排序、完整设置、数据目录迁移、备份恢复、PDF 阅读增强、生产力统计与统一阅读资料入口均已接通。
- “笔记与标注”不再只绑定当前 EPUB：统一汇总全部本地书籍的划线、笔记和页级批注，以及已连接 Kindle 的 `documents\My Clippings.txt`。
- 阅读资料支持按本地/Kindle 来源筛选、全文搜索、Markdown/纯文本导出；本地记录可回到原书定位并逐条删除。
- Kindle 笔记删除仅改写 `My Clippings.txt` 中对应记录，不修改书籍侧车数据库或云端同步标注；写入前保留安全边界并使用临时文件替换。
- PDF 支持本地文本提取、全文搜索、页面进度、书签、页面笔记以及 AI 上下文检索；EPUB/AZW3/MOBI 延续原有转换与阅读流程。
- 应用设置持久化到本地，可配置默认阅读排版、默认打开格式、Calibre 路径、AI/网络权限和数据目录；数据目录迁移通过 `.kkindle` 备份中转，API Key 与 SMTP 密码不进入备份包。

## 63. Kindle 设备字体与字典管理（2026-08-10）

- 字体、字典管理明确指 Kindle 设备资源，而不是仅管理本地阅读器资源。
- Kindle 字体支持读取、导入、导出和删除设备 `fonts` 目录中的 TTF/OTF 文件。
- Kindle 字典支持读取、导入、导出和删除设备 `documents\dictionaries` 目录中的 AZW/AZW3/MOBI/KFX 文件。
- USB 磁盘和 WPD/MTP Kindle 均支持上述操作；路径被限制在目标资源目录内，传输取消或设备断开时会清理本次未完成文件。
- Core、USB 与 WPD 路径均有回归测试，覆盖非法路径拒绝、重名处理、传输中断清理和资源类型限制。

## 64. README、MIT 许可、安装包与 GitHub 自动发版（2026-08-10）

- README 已重写主要功能说明，加入书库、Kindle 设备资源、设置和阅读资料中心截图，并明确本地与 Kindle 划线笔记、设备字体/字典读写能力。
- 根目录新增 MIT `LICENSE`；应用发布目录和安装包均携带许可证。内置字体及 Calibre 继续遵循各自第三方许可证。
- `scripts\Build-Release.ps1` 统一生成 `Kkindle-X.Y.Z-win-x64-setup.exe`、便携版 ZIP 和 `SHA256SUMS.txt`；版本号会写入程序集和安装包元数据。
- `installer\Kkindle.iss` 使用 Inno Setup 生成当前用户安装包，默认目录为 `%LOCALAPPDATA%\Programs\Kkindle`，支持开始菜单、可选桌面快捷方式和卸载，不主动删除运行时创建的 `data`、`backups` 或 `app-root.json`。
- `.github\workflows\release.yml` 在推送 `vX.Y.Z` 或预发布标签时运行 Release 测试、安装 Calibre/Inno Setup、构建自包含发行包、生成校验文件，并使用仓库 `GITHUB_TOKEN` 创建或更新 GitHub Release。
